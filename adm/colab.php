...
?>
<script>
function popserv(oque, idc)
{
var newWin;
	newWin =window.open("pesquisa.php?idClinica="+idc+"&tabela="+oque,"kidserv",'resizable=yes,scrollbars=yes')
	return false;
}
</script>
...
if (isset($_POST['Pesquisar'])){$Pesquisar = $_POST['Pesquisar'];}
     ...
     if ($t_id){
         $result=mysqli_query($con, 'SELECT * from colaboradores where idClinica = '.$t_idClinica.' and id = '.$t_id);    
...
<p>
  ...
  <p>
  <label>IDclinica 
    <input readonly name="t_idClinica" type="text" id="t_idClinica" value="<? echo $t_idClinica; ?>" />
  </label>
  </p>
  ...
  <input type="submit" name="Pesquisar" id="Pesquisar" value="Pesquisar" onclick="return popserv('colaboradores','<? echo $t_idClinica; ?>')"/>
</p>
</form>
